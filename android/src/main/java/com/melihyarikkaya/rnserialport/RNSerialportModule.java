...
public class RNSerialportModule extends ReactContextBaseJavaModule
{
    ...
    private static SerialBuffer serialBuffer;
    private static boolean isReceivingDataInProgress;
    private static long lastReceivedDataTime;
    ...
    private UsbSerialInterface.UsbReadCallback mCallback = new UsbSerialInterface.UsbReadCallback()
    {
        ...
        public void onReceivedData(byte[] bytes)
        {
            try
            {
                ...
                if ((currentTime - lastReceivedDataTime > 1000L) &&
                    (lastReceivedDataTime != 0) &&
                    (!serialBuffer.isEmpty()))
                {
                    //TODO отправить и почистить буффер
                    byte[] bufferArray = serialBuffer.getBufferArray();
                    ...
                    {
                        promiseArray.pushInt(bufferArray[i]);
                    }

                    params.putArray(payloadKey, promiseArray);
    ...
    public RNSerialportModule(ReactApplicationContext reactContext)
    {
        ...
        isReceivingDataInProgress = false;
        lastReceivedDataTime = 0;
        fillDriverList();
    }
    ...
    public void connectDevice(String deviceName, int baudRate)
    {
        try
        {
            ...
            if (baudRate < 1)
            {
                eventEmit(onErrorEvent, createError(Definitions.ERROR_CONNECT_BAUDRATE_EMPTY,
                        Definitions.ERROR_CONNECT_BAUDRATE_EMPTY_MESSAGE));
                return;
            }
    ...
    public void getDeviceList(Promise promise)
    {
        ...
        if (devices.isEmpty())
        {
            //promise.reject(String.valueOf(Definitions.ERROR_DEVICE_NOT_FOUND), Definitions
            // .ERROR_DEVICE_NOT_FOUND_MESSAGE);
    ...
    public void setDriver(String driver)
    {
        ...
        {
            eventEmit(onErrorEvent, createError(Definitions.ERROR_DRIVER_TYPE_NOT_FOUND,
                    Definitions.ERROR_DRIVER_TYPE_NOT_FOUND_MESSAGE));
            return;
        }
